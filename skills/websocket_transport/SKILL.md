...
Use this whenever building or modifying the WebSocket transport adapter in `voicebot/crates/transport/websocket`, including server setup, frame parsing, session spawning, or the client-server protocol.
...
3. **Never import** from `voicebot/crates/core` internals — only from `voicebot/crates/common`
...
use transport_websocket::WsSession; // ← forbidden in voicebot/crates/core
